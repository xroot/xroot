...
class UserInterfaceTextPrinter():
    ...
    def __init__(self, ui_content):
        ...
        # UI text init
        self.current_player_turn = UserInterfaceText(ui_content['current_player_turn'][language_id], LINE_HEIGHT.TITLE,
                                                     True, (UI_LEFT_LIMIT, 2 * UI_INTERLIGNE))

        self.next_player_hand_header = UserInterfaceText(ui_content['next_player_hand'][language_id],
                                                         LINE_HEIGHT.NORMAL, True, (UI_LEFT_LIMIT,
                                                                                    self.current_player_turn.bottom_tiles + 1.2 + 1 * UI_INTERLIGNE))

        self.next_player_hand = UserInterfaceText("", LINE_HEIGHT.NORMAL, False,
                                                  (UI_LEFT_INDENT, self.next_player_hand_header.pos_y_tiles + 1))

        if display_next_player_hand:
            self.scores = UserInterfaceText(ui_content['scores'][language_id], LINE_HEIGHT.NORMAL, True,
                                            (UI_LEFT_LIMIT, self.next_player_hand.bottom_tiles + UI_INTERLIGNE))
        else:
            self.scores = UserInterfaceText(ui_content['scores'][language_id], LINE_HEIGHT.NORMAL, True,
                                            (UI_LEFT_LIMIT, self.current_player_turn.bottom_tiles + 1 + UI_INTERLIGNE))

        self.player_score = UserInterfaceText(ui_content['player_score'][language_id], LINE_HEIGHT.NORMAL, False,
                                              (UI_LEFT_INDENT, self.scores.bottom_tiles))

        self.previous_turn_summary = UserInterfaceText(ui_content['previous_turn_summary'][language_id],
                                                       LINE_HEIGHT.NORMAL, True, (UI_LEFT_LIMIT,
                                                                                  self.scores.bottom_tiles + (0.8 * len(
                                                                                      players_names)) + UI_INTERLIGNE))

        self.word_and_points = UserInterfaceText(ui_content['word_and_points'][language_id], LINE_HEIGHT.NORMAL, False,
                                                 (UI_LEFT_INDENT, self.previous_turn_summary.bottom_tiles))

        self.nothing_played = UserInterfaceText(ui_content['nothing_played'][language_id], LINE_HEIGHT.NORMAL, False, (
        UI_LEFT_LIMIT, self.scores.bottom_tiles + (0.8 * len(players_names)) + UI_INTERLIGNE))

        self.scrabble_obtained = UserInterfaceText(ui_content['scrabble_obtained'][language_id], LINE_HEIGHT.NORMAL,
                                                   False, (UI_LEFT_INDENT, self.previous_turn_summary.bottom_tiles))

        self.remaining_letters = UserInterfaceText(ui_content['remaining_letters'][language_id], LINE_HEIGHT.NORMAL,
                                                   False, (UI_LEFT_LIMIT, self.previous_turn_summary.bottom_tiles))

        self.remaining_letter = UserInterfaceText(ui_content['remaining_letter'][language_id], LINE_HEIGHT.NORMAL,
                                                  False, (UI_LEFT_LIMIT, self.previous_turn_summary.bottom_tiles))

        self.no_remaining_letter = UserInterfaceText(ui_content['no_remaining_letter'][language_id], LINE_HEIGHT.NORMAL,
                                                     False, (UI_LEFT_LIMIT, self.previous_turn_summary.bottom_tiles))
        ...
        self.double_letter = UserInterFacePopUp(ui_content['double_letter'][language_id], LINE_HEIGHT.POP_UP, False,
                                                (0, 0), COLOR.BLACK, COLOR.BLUE_LIGHT)
        self.triple_letter = UserInterFacePopUp(ui_content['triple_letter'][language_id], LINE_HEIGHT.POP_UP, False,
                                                (0, 0), COLOR.BLACK, COLOR.BLUE_DEEP)

        self.double_word = UserInterFacePopUp(ui_content['double_word'][language_id], LINE_HEIGHT.POP_UP, False, (0, 0),
                                              COLOR.BLACK, COLOR.RED_LIGHT)
        self.triple_word = UserInterFacePopUp(ui_content['triple_word'][language_id], LINE_HEIGHT.POP_UP, False, (0, 0),
                                              COLOR.BLACK, COLOR.RED_DEEP)

    # Draw UI text
    def drawText(self):

        # Current player hand
        text = self.current_player_turn.font.render(
            self.current_player_turn.text.replace('<CURRENT_PLAYER>', var.current_player.name), 1, COLOR.GREY)
        ...
        if display_next_player_hand:
            ...
            text = self.next_player_hand_header.font.render(
                self.next_player_hand_header.text.replace('<NEXT_PLAYER>', var.current_player.next().name), 1,
                COLOR.GREY)
        ...
        for player in PLAYERS:
            if (player == var.current_player):
                ...
                if var.predicted_score == 0:  # move does not give points
                    text = self.player_score.font.render(
                        self.player_score.text.replace('_', ' ').replace('<PLAYER>', player.name).replace('<SCORE>',
                                                                                                          str(player.score)),
                        1, COLOR.BLUE_SUPER_LIGHT)
                else:
                    text = self.player_score.font.render(
                        self.player_score.text.replace('_', ' ').replace('<PLAYER>', player.name).replace('<SCORE>',
                                                                                                          str(player.score) + " (+" + str(
                                                                                                              var.predicted_score)) + ")",
                        1, COLOR.BLUE_SUPER_LIGHT)
        ...
        if len(var.last_words_and_scores) > 0:
            ...
            # header
            text = self.previous_turn_summary.font.render(
                self.previous_turn_summary.text.replace('<PREVIOUS_PLAYER>', var.current_player.previous().name), 1,
                COLOR.GREY)
            ...
            for association in var.last_words_and_scores:
                if association[0] == "!! SCRABBLE !!":
                    text = self.scrabble_obtained.font.render(self.scrabble_obtained.text.replace('<PREVIOUS_PLAYER>',
                                                                                                  var.current_player.previous().name).replace(
                        '<SCRABBLE_POINTS>', str(var.points_for_scrabble)), 1, COLOR.RED_DEEP)
                    window.blit(text, (
                    self.scrabble_obtained.pos_x, self.scrabble_obtained.pos_y + (pos_y_delta * var.tile_size)))
                else:
                    text = self.word_and_points.font.render(
                        self.word_and_points.text.replace('<WORD>', association[0]).replace('<POINTS>',
                                                                                            str(association[1])), 1,
                        COLOR.GREY)
                    window.blit(text, (
                    self.word_and_points.pos_x, self.word_and_points.pos_y + (pos_y_delta * var.tile_size)))
                pos_y_delta += 0.8

        else:
            # nothing played
            text = self.nothing_played.font.render(
                self.nothing_played.text.replace('<PREVIOUS_PLAYER>', var.current_player.previous().name), 1,
                COLOR.GREY)
        ...
        if len(var.bag_of_letters) == 0:
            ...
            if len(var.last_words_and_scores) > 0:
                window.blit(text, (self.no_remaining_letter.pos_x,
                                   self.no_remaining_letter.pos_y + (pos_y_delta + UI_INTERLIGNE) * var.tile_size))
            else:
                window.blit(text, (
                self.no_remaining_letter.pos_x, self.nothing_played.pos_y + (2 * UI_INTERLIGNE) * var.tile_size))

        elif len(var.bag_of_letters) == 1:
            text = self.remaining_letter.font.render(self.remaining_letter.text, 1, COLOR.GREY)

            if len(var.last_words_and_scores) > 0:
                window.blit(text, (self.remaining_letter.pos_x,
                                   self.remaining_letter.pos_y + (pos_y_delta + UI_INTERLIGNE) * var.tile_size))
            else:
                window.blit(text, (
                self.remaining_letter.pos_x, self.nothing_played.pos_y + (2 * UI_INTERLIGNE) * var.tile_size))

        else:
            text = self.remaining_letters.font.render(
                self.remaining_letters.text.replace('<LETTERS_REMAINING>', str(len(var.bag_of_letters))), 1, COLOR.GREY)

            if len(var.last_words_and_scores) > 0:
                window.blit(text, (self.remaining_letters.pos_x,
                                   self.remaining_letters.pos_y + (pos_y_delta + UI_INTERLIGNE) * var.tile_size))
            else:
                window.blit(text, (
                self.remaining_letters.pos_x, self.nothing_played.pos_y + (2 * UI_INTERLIGNE) * var.tile_size))
...
def resizeWindow(width, height, fullscreen, resizable, resolution_auto, custom_window_height, double_buffer,
        ...
        if delta_x == 0:
            ...
            for it_y in range(start_y, end_y + 1):
                ...
                if (it_x, it_y) in (letters_played):  # prevent to count already existing words
                    ...
                    condition_2 = ((it_x + 1) <= TILES_PER_LINE - 1) and (
                                var.current_board_state[it_x + 1][it_y] != '?')

                    if (condition_1 or condition_2):
                        logging.debug('VERTICAL WORD')

                        while (((it_x - 1) >= 0) and (
                                var.current_board_state[it_x - 1][it_y] != '?')):  # go to the begining of the word
                            it_x = it_x - 1
                        ...
                        while (((it_x) <= TILES_PER_LINE - 1) and (
                                var.current_board_state[it_x][it_y] != '?')):  # go to the end of the word
            ...
            for it_x in range(start_x, end_x + 1):
                ...
                if (it_x, it_y) in (letters_played):  # prevent to count already existing words
                    ...
                    condition_2 = ((it_y + 1) <= TILES_PER_LINE - 1) and (
                                var.current_board_state[it_x][it_y + 1] != '?')

                    if (condition_1 or condition_2):
                        logging.debug('HORIZONTAL WORD')

                        while (((it_y - 1) >= 0) and (
                                var.current_board_state[it_x][it_y - 1] != '?')):  # go to the begining of the word
                            it_y = it_y - 1
                        ...
                        while (((it_y) <= TILES_PER_LINE - 1) and (
                                var.current_board_state[it_x][it_y] != '?')):  # go to the end of the word
...
logging.basicConfig(filename=path_log_file, filemode='w', level=logging.DEBUG,
                    format='%(asctime)s.%(msecs)03d  |  %(levelname)s  |  %(message)s', datefmt='%Y-%m-%d %p %I:%M:%S')
...
window = resizeWindow(width, height, cfg_fullscreen, cfg_resizable, cfg_resolution_auto, cfg_custom_window_height,
                      cfg_double_buffer, cfg_hardware_accelerated)
...
if game_is_running:
    ...
    # create hand_holder
    hand_holder = Hand_holder("hand_holder", UI_LEFT_LIMIT - 0.1,
                              ui_text.current_player_turn.pos_y_tiles + 0.9)  # automatically stored in the corresponding layer
    ...
    button_end_turn = Button("end_turn", tiles1(hand_holder.rect.x) + var.number_of_letters_per_hand + 0.2 + 0.75,
                             ui_text.current_player_turn.pos_y_tiles + 1)
    button_draw_tiles = Button("draw", tiles1(hand_holder.rect.x) + var.number_of_letters_per_hand + 0.2 + 0.75,
                               button_end_turn.pos_y + 1.25)
...
while game_is_running:
    ...
    for event in pygame.event.get():
        ...
        # TODO create a specific function ?
            ...
            window = resizeWindow(var.monitor_resolution.current_w, var.monitor_resolution.current_h, cfg_fullscreen,
                                  cfg_resizable, cfg_resolution_auto, cfg_custom_window_height, cfg_double_buffer,
                                  cfg_hardware_accelerated)
            pygame.draw.rect(window, (0, 0, 0),
                             ((0, 0), (var.monitor_resolution.current_w, var.monitor_resolution.current_h)))
            pygame.display.update()

            window = resizeWindow(width, height, cfg_fullscreen, cfg_resizable, cfg_resolution_auto,
                                  cfg_custom_window_height, cfg_double_buffer, cfg_hardware_accelerated)
        ...
        # ~~~~~~~~~~~ MOUSE BUTTONS ~~~~~~~~~~~
                ...
                if var.current_action == 'SELECT_A_LETTER':
                    ...
                    for letter_from_hand in var.current_player.hand:
                        ...
                        if letter_from_hand.rect.collidepoint(cursor_pos_x, cursor_pos_y) == True:
                            var.delta_pos_on_tile = (
                            cursor_pos_x - letter_from_hand.rect.x, cursor_pos_y - letter_from_hand.rect.y)
                    ...
                    for letter_from_board in layers.letters_just_played:
                        ...
                        if letter_from_board.rect.collidepoint(cursor_pos_x, cursor_pos_y) == True:
                            var.delta_pos_on_tile = (
                            cursor_pos_x - letter_from_board.rect.x, cursor_pos_y - letter_from_board.rect.y)
                        ...
                        for hand_holder in layers.hand_holder:
                            ...
                            if hand_holder.rect.collidepoint(cursor_pos_x, cursor_pos_y) == True:
                                ...
                                if var.current_player.hand_state[index_in_hand] == 0:
                                    ...
                                    delta_x, delta_y = layers.hand_holder.sprites()[0].pos_x + 0.1, \
                                                       layers.hand_holder.sprites()[0].pos_y + 0.1
            ...
            # ~~~~~~~~~~~ RELEASE LEFT CLIC ~~~~~~~~~~~
                ...
                if var.current_action == 'SELECT_A_LETTER':
                    ...
                    if ((button_end_turn.rect.collidepoint(cursor_pos_x, cursor_pos_y) == True) and (
                    button_end_turn.is_pushed)):
                        ...
                        while len(var.bag_of_letters) > 0 and index_hand < var.number_of_letters_per_hand:
                            if var.current_player.hand_state[index_hand] == 0:
                                ...
                                delta_x, delta_y = tiles1(
                                    hand_holder.rect.x), ui_text.current_player_turn.pos_y_tiles + 1
        ...
        # ~~~~~~ MOUSE MOTION ~~~~~~
            ...
            if var.current_action == 'SELECT_A_LETTER':
                ...
                for button in layers.buttons:
                    if (button.rect.collidepoint(cursor_pos_x, cursor_pos_y) == True) and (
                    not button.is_highlighted) and (not button.is_pushed):
                        button.turnOnHighlighted()
                        buttons_changed = True
                    elif (button.rect.collidepoint(cursor_pos_x, cursor_pos_y) == False) and (
                    button.is_highlighted) and (not button.is_pushed):
                        button.turnOffHighlighted()
                        buttons_changed = True
            ...
            if len(layers.selected_letter) == 1:
                layers.selected_letter.sprites()[0].moveAtPixels(cursor_pos_x - var.delta_pos_on_tile[0],
                                                                 cursor_pos_y - var.delta_pos_on_tile[1])
                            ...
                            if (tile.name != 'empty'):
                                ...
                                if tile.id != ui_text.id_tile_pop_up:
                                    ...
                                    if cursor_on_empty_tile:
                                        ...
                                        ui_text.drawHelpPopPup(tile, tile.rect.x + ((2 / 60.0) * var.tile_size),
                                                               tile.rect.y + var.tile_size - (2 / 60.0) * (
                                                                   var.tile_size))
                                        pygame.display.update()
